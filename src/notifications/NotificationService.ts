...
export class NotificationService {
  ...
  private getDefaultConfig(
    ...
    if (vscodeConfig.get("notifications.style") !== undefined) {
      config.style = vscodeConfig.get("notifications.style") as
        | "native"
        | "toast"
        | "both";
    }
    if (vscodeConfig.get("notifications.position") !== undefined) {
      config.position = vscodeConfig.get("notifications.position") as
        | "top-right"
        | "bottom-right"
        | "top-left"
        | "bottom-left";
  ...
  private setupEventListeners(): void {
    ...
    const unsubscribeAchievement = this.eventBus.on(
      FOCUS_EVENTS.ACHIEVEMENT_UNLOCKED,
      (data) => {
    ...
    const unsubscribeLevelUp = this.eventBus.on(
      FOCUS_EVENTS.LEVEL_UP,
      (data) => {
    ...
    const unsubscribePomodoro = this.eventBus.on(
      FOCUS_EVENTS.POMODORO_COMPLETED,
      (data) => {
    ...
    // Goal notifications
    const unsubscribeGoal = this.eventBus.on(
      FOCUS_EVENTS.GOAL_COMPLETED,
      (data) => {
    ...
    const unsubscribeDeepWorkStart = this.eventBus.on(
      FOCUS_EVENTS.DEEP_WORK_STARTED,
      (data) => {
    ...
    const unsubscribeDeepWorkEnd = this.eventBus.on(
      FOCUS_EVENTS.DEEP_WORK_ENDED,
      (data) => {
        if (this.config.enabled && this.config.types.deepWork) {
      ...
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration("focusPulse.notifications")) {
          ...
          this.outputChannel.appendLine(
            `[Config] Notification configuration updated: ${JSON.stringify(this.config)}`,
          );
        }
      }),
    );
  ...
  private handleAchievementUnlocked(data: any): void {
    const defaultMessage = DEFAULT_NOTIFICATION_MESSAGES[
      ...
      actions: [
        {
          ...
          action: () =>
            vscode.commands.executeCommand("focusPulse.openDashboard"),
          style: "primary",
        },
      ],
    });
  ...
  private handleLevelUp(data: any): void {
    const defaultMessage = DEFAULT_NOTIFICATION_MESSAGES["xp:level_up"](
      data.newLevel,
    );
    this.showNotification({
      id: `levelup-${Date.now()}`,
  ...
  private handlePomodoroCompleted(data: any): void {
    const defaultMessage =
      DEFAULT_NOTIFICATION_MESSAGES["pomodoro:completed"]();
    this.showNotification({
      ...
      actions: [
        {
          ...
          label: "Start Break",
          action: () =>
            vscode.commands.executeCommand("focusPulse.pomodoroToggle"),
          style: "secondary",
        },
      ],
    });
  ...
  private handleGoalCompleted(data: any): void {
    ...
    const defaultMessage =
      DEFAULT_NOTIFICATION_MESSAGES["goal:completed"](goalType);
    this.showNotification({
      id: `goal-${Date.now()}`,
      type: "success",
      title: defaultMessage.title,
  ...
  private handleDeepWorkEnded(data: any): void {
    const defaultMessage =
      DEFAULT_NOTIFICATION_MESSAGES["deepwork:completed"]();
    this.showNotification({
  ...
  public showNotification(notification: NotificationMessage): void {
    ...
    this.outputChannel.appendLine(
      `[Notification] ${notification.type}: ${notification.title} - ${notification.message}`,
    );
  ...
  public testNotification(
    type: "native" | "toast" | "both" = this.config.style,
  ): void {
    this.showNotification({
      id: `test-${Date.now()}`,
      type: "info",
      title: "Terster notification",
      message:
      ...
      actions: [
        {
          ...
          action: () =>
            vscode.window.showInformationMessage(
              "The notification is working perfectly!",
            ),
          style: "primary",
        },
      ],
