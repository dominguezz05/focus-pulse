...
export class AchievementsComponent implements DashboardComponent {
    ...
    if (achievementsEl) {
      ...
      if (!achievements.length) {
        ...
        achievements.forEach(a => {
          ...
          span.innerHTML = 
            '<span class="text-[11px] font-semibold text-emerald-300">' + a.title + 
            '</span><span class="text-[10px] text-emerald-200/80">' + a.description + '</span>';
  ...
  private buildAllAchievements(all: any[]): void {
    ...
    if (!all || !all.length) {
      ...
      p.textContent = 'Todavía no hay catálogo de logros disponible.';
